   ...
   <!--  #### 리스트  start ####-->
	...
	<div class="notice_writer_info">
		<div class="notice_writer_image">
			...
			<img src="${nwriterList[0].uploadfile}">
		</div>
		<div class="notice_writer_nname">${nwriterList[0].nname}님의 게시물	</div>
	</div>
	...
	<table class="ntable"> 
		...
		<c:if test="${not empty nwriterList}">
			<c:forEach  var="nwriterList" items="${nwriterList}" >
			<tr class="ntable_tr">
				<td>${nwriterList.nno}</td>
				<td>${nwriterList.subject}</td>
				<td><a href="ndetail?nno=${nwriterList.nno}">${nwriterList.title}</a></td>
				<td>${nwriterList.nname}</td>
				<td>${nwriterList.regdate}</td><td>${nwriterList.cnt}</td>
			</tr>
			</c:forEach>
		</c:if>
	</table>
	...
	<div align="center">
		...
		<c:choose>
			<c:when test="${pageMaker.prev && pageMaker.spageNo>1}">
				<a href="nwriterList${pageMaker.searchQuery(1)}">&lt;&lt;</a>&nbsp;     
				<a href="nwriterList${pageMaker.searchQuery(pageMaker.spageNo-1)}">&lt;</a>&nbsp;&nbsp;  
			</c:when>
			<c:otherwise>
		...
		<c:forEach var="i" begin="${pageMaker.spageNo}" end="${pageMaker.epageNo}">
			...
			<c:if test="${i != pageMaker.cri.currPage}">
				<a href="nwriterList${pageMaker.searchQuery(i)}">${i}</a>&nbsp;
		...
		<c:choose>
			<c:when test="${pageMaker.next && pageMaker.epageNo>0}">
				<a href="nwriterList${pageMaker.searchQuery(pageMaker.epageNo+1)}">&nbsp;&gt;</a>     
				<a href="nwriterList${pageMaker.searchQuery(pageMaker.lastPageNo)}">&nbsp;&gt;&gt;</a> 
				-->
			</c:when>
			<c:otherwise>
				<font color="Gray">&nbsp;&gt;&gt;</font>   
